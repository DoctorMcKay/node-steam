...
prototype.getSteamLevel = function(steamids, callback) {
  ...
  this._send(EMsg.ClientFSGetFriendsSteamLevels | protoMask, schema.CMsgClientFSGetFriendsSteamLevels.serialize({
...
handlers[EMsg.ClientClanState] = function(data) {
  ...
  if (clanState.nameInfo) {
    this.emit('groupData', clanState);
    this.groupsData[clanState.steamidClan] = clanState;
  }
  
  if (clanState.events)
    this.emit('event', clanState.steamidClan, clanState.events[0].headline, clanState.events[0].eventTime, clanState.events[0].gid);
  
  if (clanState.announcements)
    this.emit('announcement', clanState.steamidClan, clanState.announcements[0].headline, clanState.announcements[0].gid); // TODO: more data
};

handlers[EMsg.ClientFSGetFriendsSteamLevelsResponse] = function(data, callback) {
  var friends = schema.CMsgClientFSGetFriendsSteamLevelsResponse.parse(data).friends;
  var output = {};
