...
prototype.picsGetProductInfo = function(apps, packages, callback) {
  ...
  this._send(EMsg.PICSProductInfoRequest | protoMask, schema.CMsgPICSProductInfoRequest.serialize({
    packages: packages,
    apps: apps
  }), callback);
};
...
prototype.picsGetAccessToken = function(apps, packages, callback) {
  this._send(EMsg.PICSAccessTokenRequest | protoMask, schema.CMsgPICSAccessTokenRequest.serialize({
    "packageids": packages || [],
    "appids": apps || []
  }), callback);
};
...
handlers[EMsg.PICSChangesSinceResponse] = function(data, callback) {
  var proto = schema.CMsgPICSChangesSinceResponse.parse(data);
  callback(proto);
};

handlers[EMsg.PICSProductInfoResponse] = function(data, callback) {
  var proto = schema.CMsgPICSProductInfoResponse.parse(data);
  var apps = {};
...
handlers[EMsg.PICSAccessTokenResponse] = function(data, callback) {
  var proto = schema.CMsgPICSAccessTokenResponse.parse(data);
  var apps = {};
