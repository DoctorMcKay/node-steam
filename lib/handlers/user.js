...
prototype.requestFreeLicense = function(appIDs, callback) {
  this._send(EMsg.ClientRequestFreeLicense | protoMask, new schema.CMsgClientRequestFreeLicense({
    appids: appIDs
  }), callback);
};
...
handlers[EMsg.ClientRequestFreeLicenseResponse] = function(data, callback) {
  if(!callback) {
    return;
  }
  
  var response = schema.CMsgClientRequestFreeLicenseResponse.decode(data);
  callback(response.eresult, response.grantedPackageids, response.grantedAppids);
};

handlers[EMsg.ClientLicenseList] = function(data) {
  var proto = schema.CMsgClientLicenseList.decode(data);
  if(proto.eresult == Steam.EResult.OK) {
    this.emit('licenses', proto.licenses);
    this.licenses = proto.licenses;
  }
};
